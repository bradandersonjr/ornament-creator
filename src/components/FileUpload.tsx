  ...
  if (showContent) {
    ...
    return (
      <div className="w-full">
        <div className="mb-4 flex justify-between items-center bg-base-200 p-4 rounded-lg sticky top-0 z-10">
          <button className="btn btn-primary" onClick={handleBack}>
            Back
          </button>
          <p className="text-base-content truncate">{currentPath}</p>
        </div>
        <div className="mt-4">
          {folders.length > 0 && (
  ...
  return (
    <div className="flex w-full items-center justify-center">
      <label
        ...
        className="flex h-[512px] w-[512px] cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-base-300 bg-base-200 hover:bg-base-300 transition-colors"
        onClick={() => handleFolderSelect()}
      >
        <div className="flex flex-col items-center justify-center pb-6 pt-5">
          <svg
            className="mb-4 h-8 w-8 text-base-content"
            aria-hidden="true"
          ...
          <p className="text-xs text-base-content">Select a folder containing files</p>
        </div>
      </label>
    </div>
  );
